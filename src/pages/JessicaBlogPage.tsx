...
const JessicaBlogPage: React.FC = () => {
  ...
  // Récupération des posts depuis backend et store
  ...
  useEffect(() => {
    ...
    const storedPosts = blogStore.list().filter(r => r.status === 'published');
    staticPosts = storedPosts.map((r, i) => ({
      id: `adm-${i + 1}`,
      slug: r.slug,
      title: r.title,
      excerpt: r.excerpt || '',
  ...
  // Catégories et icônes Jessica-style
  ...
  const filtered = useMemo(() => {
    return blogPosts.filter(p => {
      ...
      const okQ = !q || p.title.toLowerCase().includes(qq) || p.excerpt.toLowerCase().includes(qq) || p.tags.some(tg => tg.toLowerCase().includes(qq));
  ...
  const getCoverFor = (slug: string, idx: number) => landingImageBySlug[slug] || blogPosts.find(p => p.slug === slug)?.image || imageForIndex(idx);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <section className="sticky top-[84px] z-30 mb-12">
          <div className="rounded-2xl bg-white/80 backdrop-blur-sm border border-[#f4a6a9]/30 shadow-lg p-6 flex flex-wrap gap-4 items-center">
            ...
            />
            <select value={category} onChange={e => { setCategory(e.target.value); setPage(1); }} className="h-12 px-4 rounded-full border border-[#f4a6a9]/30 bg-white text-sm">
              <option value="all">All Stories</option>
              {Object.keys(categoryMeta).map(c => <option key={c} value={c}>{getCategoryName(c)}</option>)}
            </select>
            <select value={year} onChange={e => { setYear(e.target.value ? Number(e.target.value) : ''); setPage(1); }} className="h-12 px-4 rounded-full border border-[#f4a6a9]/30 bg-white text-sm w-[120px]">
              <option value="">All Years</option>
              {years.map(y => <option key={y} value={y}>{y}</option>)}
            </select>
            <select value={sort} onChange={e => { setSort(e.target.value as any); setPage(1); }} className="h-12 px-4 rounded-full border border-[#f4a6a9]/30 bg-white text-sm">
              <option value="newest">Latest First</option>
              <option value="oldest">Oldest First</option>
            </select>
            {(q || category !== 'all' || year || tags.length || sort !== 'newest') && (
              <button onClick={resetAll} className="h-12 px-6 rounded-full bg-[#f4a6a9] text-white text-sm font-medium hover:bg-[#e89396] transition-colors">
                Reset All
              </button>
            )}
          </div>
        ...
        <section id="posts" aria-live="polite" className="min-h-[400px]">
          {paginated.length === 0 && (
            <div className="text-center py-24">
              ...
              <button onClick={resetAll} className="inline-flex items-center px-6 py-3 bg-[#f4a6a9] text-white rounded-full font-medium hover:bg-[#e89396] transition-colors">Show All Stories</button>
            </div>
          )}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex gap-1">
                        {post.tags.slice(0, 2).map(tag => <span key={tag} className="px-2 py-1 bg-[#f4a6a9]/10 text-[#f4a6a9] text-xs rounded-full">{tag}</span>)}
                      </div>
                      <button onClick={() => navigate(`/blog/${post.slug}`, { state: { coverOverride: getCoverFor(post.slug, idx), fromJessica: true } })} className="inline-flex items-center px-4 py-2 bg-[#f4a6a9] text-white text-sm font-medium rounded-full hover:bg-[#e89396] transition-colors group-hover:shadow-lg">Read Story →</button>
                    </div>
                  </div>
                </article>
              );
